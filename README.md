...
```
git clone https://github.com/evgenii-poliakoff/lcsolver.git
cd lcsolver
```
...
#### When using lcsolver package in VS Code jupyter notebooks

When running jupyter notebooks from VS Code, the virtual environment is activated by clicking on "Select Kernel", "Select Another Kernel ...", "Jupyter Kernel ...", "lcsolver-env (Python 3.XX.XX) conda-env/bin/python3". 

#### When using lcsolver package in JupyterLab notebooks

When running jupyter notebooks from JupyterLab web interface, the virtual environment is activated by clicking on "Python 3 (ipykernel)", "Select kernel", and choosing "lcsolver-env" in the dropdown list, then "Select".
