...
from lcsolver.tools import mv
import time
from lcsolver import evolution_chained2_kicked
import math
import random
import pathlib
import scipy.sparse
...
class bath:
    ...
    def find_interval(self, ti):
        ...
        raise Exception('Time moment ', ti, ' exceeds maximal time limit ', self.intervals[-1][1])
    ...
    def save_to_file(self, fout):
        ...
        p = os.path.join(os.getcwd(), fout, "time.txt")
        os.makedirs(os.path.dirname(p), exist_ok = True)
        with open(p, "w") as f:
            for i in range(self.nt):
                print(str(self.t[i]), file = f)
        
        p = os.path.join(os.getcwd(), fout, "intervals.txt")
        os.makedirs(os.path.dirname(p), exist_ok = True)
        with open(p, "w") as f:
            for i in self.intervals:
                a = i[2]
        ...
        p = os.path.join(os.getcwd(), fout, "couplings.txt")
        os.makedirs(os.path.dirname(p), exist_ok = True)
        with open(p, "w") as f:
            for i in self.intervals:
                b = i[2]
        ...
        p = os.path.join(os.getcwd(), fout, "rotations.txt")
        os.makedirs(os.path.dirname(p), exist_ok = True)
        with open(p, "w") as f:
    ...
    def load_from_file(cls, fin):
        ...
        try:
            p = os.path.join(os.getcwd(), fin, "intervals.txt")
            with open(p) as f:
                while True:
        ...
        try:
            p = os.path.join(os.getcwd(), fin, "couplings.txt")
            with open(p) as f:
                while True:
        ...
        p = os.path.join(os.getcwd(), fin, "rotations.txt")
        with open(p) as f:
        ...
        p = os.path.join(os.getcwd(), fin, "time.txt")
        with open(p) as f:
            tg = np.loadtxt(f)
        ntg = tg.size
        dt = tg[1] - tg[0]
    ...
    def from_chain(cls, chain, n_sites = -1, rel_tol = 10**(-3), ring_size = 4, t_max = 100, dt = 0.01, visualize = False):
        ...
        for i in intervals_in:
            ...
            if n_coupled > max_n_coupled:
                ...
                if visualize:
                    u_ = U.T.conj()
                    U_cdia[n_out : n_in, :] = u_ @ U_cdia[n_out : n_in, :]
                
                    duration = (end - begin) * dt
                    e_, v_ = eig(u_)
                    e_ = np.log(e_ + 0j) / duration
                    h_ = v_ @ np.diag(e_) @ v_.conj().T
                    
                    bth.U_cdia_list.append((np.copy(U_cdia), n_out, n_in, u_, h_, begin, end))
                ...
                if visualize:
                    bth.U_cdia_list.append((np.copy(U_cdia), n_out, n_in, None, None, begin, end))
        ...
        for i in intervals_out:
            ...
            if duration < min_duration:
                print("Warning: decrease time step: the interval [", i[0], ", ", i[1], "] has too small duration: ", duration)
            
            while duration < min_duration and False:
...
def job(image, opts):
    ...
    if psi0 is None:
        ...
        psi_ini_e = np.zeros(dim_e, dtype = complex)
        psi_ini_e[0] = 1
        psi_ini = np.kron(psi0, psi_ini_e)
    ...
    if not Os is None:
        av_O = [(np.zeros(nt_, dtype = complex), o_[0], tools.kron(o_[1], id_e)) for o_ in Os]
    
    if not Oj is None:
        for o_ in Oj:
            av_O.append((np.zeros(nt_, dtype = complex), o_[0], o_[1]))
    ...
    if not Pj is None:
        av_P = []
        for p_ in Pj:
            av_P.append((np.zeros(nt_, dtype = complex), p_[0], p_[1]))
    ...
    def eval_o(ti, psi):
        ...
        if not av_O is None:
            for av in av_O:
                mv(av[2], psi, psi_buff)
                av[0][ti] = av[0][ti] + np.vdot(psi, psi_buff)
                
        if not av_P is None:
            for av in av_P:
                av[0][ti] = av[0][ti] + np.vdot(av[2], psi_buff)
        ...
        for (i, i_) in zip(intervals, [None] + intervals[:-1]):
            ...
            if not i_ is None:
                ...
                for q in range(a_, a):
                    ...
                    if opts.sampling_type == SamplingType.schmidt_jumps:
                    ...
                    if opts.sampling_type == SamplingType.vacuum_projection:
                        
                        psi_begin = lo.project_to_vacuum(psi_begin, measured_mode)
    ...
    if not av_O is None:    
        for i in range(len(av_O)):
            path = os.path.join(os.getcwd(), fout, av_O[i][1] + "_" + str(image) + ".txt")
    ...
    if not av_P is None:    
        for i in range(len(av_P)):
            path = os.path.join(os.getcwd(), fout, av_P[i][1] + "_" + str(image) + ".txt")
            with open(path, "w") as f:
                av = av_P[i]
                print(str(csize) + " " + str(0), file = f)
                for i in range(nt_):
    ...
    if compute_probs:
        ...
        path = os.path.join(os.getcwd(), fout, "probs" + "_" + str(image) + ".txt")
        np.savetxt(path, pa)
        
    if compute_dmatrices:
        path = os.path.join(os.getcwd(), fout, "dmatrices" + "_" + str(image) + ".txt")
        with open(path, "w") as f:
            dm = dm_chunk[0]
            dm_ = dm.flatten()
            ds = len(dm_)
...
class problem:
    ...
    def __init__(self):
        ...
        self.compute_dmatrices = False
        self.psi0 = None
        self.sampling_type = SamplingType.schmidt_jumps
    ...
    def set_sampling_type(self, st):
        self.sampling_type = st
    ...
    def add_joint_observable(self, name, Oj):
        self.joint_observables.append((name, Oj))
        
    def add_projection(self, name, Pj):
        self.projections.append((name, Pj))
        
    def set_impurity_initial_state(self, psi):
        self.psi0 = psi
    ...
    def solve(self, cfrom = 1, cto = 1, csize = 1, maxcores = 4, tmax = 100.0, fout = 'results'):
        ...
        o.Oj = self.joint_observables
        o.projections = self.projections
        ...
        o.psi0 = self.psi0
        o.sampling_type = self.sampling_type
        ...
        for o_ in self.joint_observables:
            res = load_results(o_[0], [cfrom, cto + 1], fout)
            results[o_[0]] = res.o
        ...
        for p_ in self.projections:
            res = load_results(p_[0], [cfrom, cto + 1], fout)
            results[p_[0]] = res.o
