...
class LocalObservables:
    ...
    def local_projections_f(self, f, m_max, n_max, id_s):
        ...
        from lcsolver import secondquant
        from lcsolver import local_op
...
class spin_boson_model:
    def __init__(self, num_modes, max_num_quanta):
        import lcsolver.secondquant as sq
...
class spin_chain:
    def __init__(self, num_modes):
        import lcsolver.secondquant as sq
        m = num_modes
        space = sq.fock_space(num_modes = m, max_total_occupation = 2**m, statistics = 'Bose', max_local_occupations = 1)
        
        self.space = space
        
        self.sp = space.annihilate
        self.sm = space.create
        
        self.sx = [space.sigmax(i) for i in range(m)]
        self.sy = [space.sigmax(i) for i in range(m)]
        self.sz = [space.sigmax(i) for i in range(m)]
